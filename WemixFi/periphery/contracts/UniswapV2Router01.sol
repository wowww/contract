...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function addLiquidityWEMIX(
        ...
        address to,
        uint deadline
    ) external override payable ensure(deadline) returns (uint amountToken, uint amountWEMIX, uint liquidity) {
        ...
        address pair = UniswapV2Library.pairFor(factory, token, WWEMIX);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWWEMIX(WWEMIX).deposit{value: amountWEMIX}();
        assert(IWWEMIX(WWEMIX).transfer(pair, amountWEMIX));
        liquidity = IUniswapV2Pair(pair).mint(to);
        if (msg.value > amountWEMIX) TransferHelper.safeTransferWEMIX(msg.sender, msg.value - amountWEMIX); // refund dust WEMIX, if any
    }
    ...
    function removeLiquidityWEMIX(
        ...
        address to,
        uint deadline
    ) public override ensure(deadline) returns (uint amountToken, uint amountWEMIX) {
    ...
    function removeLiquidityWEMIXWithPermit(
        ...
        uint deadline,
        bool approveMax, uint8 v, bytes32 r, bytes32 s
    ) external override returns (uint amountToken, uint amountWEMIX) {
        address pair = UniswapV2Library.pairFor(factory, token, WWEMIX);
        uint value = approveMax ? type(uint).max  : liquidity;
        IUniswapV2Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    {
        require(path[0] == WWEMIX, 'UniswapV2Router: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWWEMIX(WWEMIX).deposit{value: amounts[0]}();
        assert(IWWEMIX(WWEMIX).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactWEMIX(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WWEMIX, 'UniswapV2Router: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWWEMIX(WWEMIX).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        if (msg.value > amounts[0]) TransferHelper.safeTransferWEMIX(msg.sender, msg.value - amounts[0]); // refund dust WEMIX, if any
    }
